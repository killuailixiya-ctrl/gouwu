...
class _ScreenshotImportPageState extends State<ScreenshotImportPage> {
  ...
  Future<void> _addSeries() async {
    ...
    if (name != null && name.isNotEmpty) {
      try {
        ...
        final seriesList = await _seriesDao.getAll();
        final newSeries = seriesList.firstWhere((s) => s.name == name);
        setState(() {
          _seriesList = seriesList;
          // 自动为未选IP的商品选中新添加的IP
          for (final item in _items) {
            item.selectedSeriesId ??= newSeries.id;
          }
        });
      } catch (e) {
        if (mounted) {
  ...
  Future<void> _addCharacter(int itemIndex, String seriesId) async {
    ...
    if (name != null && name.isNotEmpty) {
      try {
        ...
        final newChar = characterList.firstWhere((c) => c.name == name);
        setState(() {
          _items[itemIndex].characterList = characterList;
          // 自动选中新添加的角色
          _items[itemIndex].selectedCharacterId = newChar.id;
        });
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _onSeriesChanged(int itemIndex, String? seriesId) async {
    if (seriesId != null && seriesId.isNotEmpty) {
      ...
      setState(() {
        ...
        _items[itemIndex].selectedCharacterId = null;
        _items[itemIndex].characterList = [];
      });
    }
  }
      ...
      return GlassContainer(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Text('订单截图', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const Spacer(),
                Text('（选填，Web端暂不支持OCR识别）',
        ...
        children: [
          Text('订单信息', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            ...
            DropdownButtonFormField<String>(
              key: ValueKey('platform_${_platforms.length}'),
            ...
            Row(
              ...
              children: [
                TextButton.icon(
                  ...
                  label: const Text('添加平台', style: TextStyle(fontSize: 13)),
                ...
                TextButton.icon(
                  ...
                  label: const Text('管理平台', style: TextStyle(fontSize: 13)),
                  onPressed: () async {
  ...
  Widget _buildItemsSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        ..._items.asMap().entries.map((entry) => buildItemCard(entry.key, entry.value)),
      ],
    );
  }

  Future<String> getItemImageDir() async {
    final dir = await getApplicationDocumentsDirectory();
  ...
  Widget buildItemCard(int index, _ImportItemEntry item) {
    return GlassContainer(
              ...
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      DropdownButtonFormField<String>(
                        key: ValueKey('series_${index}_${_seriesList.length}'),
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          label: const Text('添加IP', style: TextStyle(fontSize: 12)),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      DropdownButtonFormField<String>(
                        key: ValueKey('character_${index}_${item.characterList.length}'),
                        ...
                        items: [
                          ...
                          ...item.characterList.map((c) => DropdownMenuItem(
                            value: c.id,
